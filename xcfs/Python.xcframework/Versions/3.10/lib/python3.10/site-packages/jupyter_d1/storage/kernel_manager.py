...
class KernelManager:
    ...
    async def _async_warmup_kernels(self, kernel_names):
        for kernel_name in kernel_names:
            self.warmup_tasks.append(
                asyncio.create_task(self.warmup(kernel_name))
            )
        await asyncio.gather(*self.warmup_tasks)
        self.kernels_warmed = True

    def warmup_kernels(self, warmup_kernel_names, prewarmed_kernels_file=None):
        """
    ...
    async def warmup(self, kernel_name=None, timeout=120):
        """
    ...
    def get_kernelspec(self, uuid) -> Tuple[str, Dict[str, Any]]:
        kernelspec = self._kmanager.get_kernel(str(uuid)).kernel_spec
        return self.get_kernelspec_by_name(kernelspec.name)
