...
# Pattern to capture ansi control codes, newlines, and carriage returns,
# for ansi control codes it also captures the parameters and the command
pattern = re.compile(r"(\x1b\[([;\d]*)([A-Za-z])|\n|\r)")
...
class NotebookManager:
    ...
    def handle_special_characters(
        ...
        """
        Appends new stream text to existing stream text, handling some ANSI
        control codes (cursor up), newlines, and carriage returns
        """
        ...
        for match in chain(re.finditer(pattern, new_text), iter([None])):
            ...
            if groups[0] == "\n":
                ...
                cursor_position = 0
            elif groups[0].startswith("\x1b["):
                # TODO: Handle things like color codes, right now we dont
                # even pass them through to the client because it would interfere
                ...
                if groups[2] == "A":
                    ...
                    if len(groups[1]) > 0:
                        try:
                            ...
                            pass
                    # Dont allow cursor movement above first line
                    if cursor_line - number > 0:
                        cursor_line -= number
                    else:
                        cursor_line = 0
